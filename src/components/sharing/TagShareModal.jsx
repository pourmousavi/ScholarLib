...
export default function TagShareModal({ tagSlug, onClose }) {
  ...
  const tag = tagRegistry[tagSlug]

  // Count documents with this tag
  const docCount = Object.values(documents).filter(
    doc => (doc.user_data?.tags || []).includes(tagSlug)
  ).length

  // Load collaborators from tag's shared_with on mount
  useEffect(() => {
    if (tag) {
      setCollaborators(tag.shared_with || [])
    }
    setIsLoading(false)
  }, [tag])
  ...
  const handleInvite = async (e) => {
    ...
    try {
      ...
      // Update tag's shared_with
      const newSharedWith = [...collaborators, newCollab]
      const result = updateTag(tagSlug, { shared_with: newSharedWith })
      ...
      showToast({ message: error.message || 'Failed to share tag', type: 'error' })
    } finally {
      setIsSending(false)
    }
  }
  ...
  const handleSendReminder = (collab) => {
    ...
    const message = `Hi, I shared the "${tag.displayName}" tag with you on ScholarLib: ${appUrl}`
    ...
    return (
      <Modal onClose={onClose} width={600}>
        <div className={styles.container}>
          ...
          <div className={styles.empty}>Tag not found</div>
        </div>
      </Modal>
    )
  }
  ...
  return (
    <Modal onClose={onClose} width={600}>
      <div className={styles.container}>
        ...
        {/* Info about tag documents */}
        <div className={styles.footer}>
          <span className={styles.hint}>
            Sharing gives access to {docCount} document{docCount !== 1 ? 's' : ''} with this tag
          </span>
        </div>
      </div>
    </Modal>
